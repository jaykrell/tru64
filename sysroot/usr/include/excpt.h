...
#ifdef _INCLUDE_ID
static char *excpt_h_id="$Header: /share/buffer/build/rcs.dsk1/os/src/usr/include/alpha/excpt.h,v 1.1.16.2 1997/05/22 17:52:42 Neil_Lewis Exp $";
#endif
...
typedef union exception_code {
    ...
    struct {
	pdsc_uint_32		facility_dependent_1:16;
	pdsc_uint_32		facility:12;
	pdsc_uint_32		facility_dependent_2:4;
    ...
    struct {
	pdsc_uint_32		code:16;	/* subcode */
	pdsc_uint_32		facility:13;	/* base distinguisher */
	pdsc_uint_32		customer:1;	/* nt versus customer */
	pdsc_uint_32		severity:2;	/* as it says */
	pdsc_uint_32            reserved;       /* sign extension of bit 31 */
    } exception_code_nt;
    struct {
	pdsc_uint_32            severity:3;     /* as it says */
	pdsc_uint_32            message_number:13;      /* subcode */
	pdsc_uint_32            facility:11;    /* base distinguisher */
	pdsc_uint_32            customer:1;     /* vms versus customer */
	pdsc_uint_32            control:4;      /* 1=>prnt,rest resrv */
	pdsc_uint_32            reserved;       /* sign extension of bit 31 */
    } exception_code_vms;
...
#define EXC_DECTHREADS_USER		(EXC_OSF|10UL)	/* DECthreads user */
#define EXC_JAVA_USER			(EXC_OSF|11UL)	/* Java user */
...
/* EXC_OTS_UNWIND and EXC_LONGJMP_UNWIND are reserved for internal use by
 * the exception handling support.
 */
#define EXC_LONGJMP_UNWIND			EXC_VALUE(EXC_INTERNAL, 13)
#define EXC_OTS_UNWIND				EXC_VALUE(EXC_INTERNAL, 14)


#define EXC_FALSE 0		/* return value from exc_dispatch_exception */
...
#else
typedef struct _sigcontext CONTEXT, *PCONTEXT;
...
;
extern struct code_range_entry * 
exc_crd_list_head
;
...
;
extern unsigned long
exc_remote_virtual_unwind(void *handle, 
			  int (*fetch_from_process)(void*, void*, void*, long),
                          exc_address crd_handle,
			  PRUNTIME_FUNCTION pcrd, PCONTEXT pcontext)
;
extern unsigned long
