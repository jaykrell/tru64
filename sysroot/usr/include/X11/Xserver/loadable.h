...
#ifndef MISC_H 
/* the following is needed from misc.h */ 
...
typedef int Bool;
#include <X11/Xfuncs.h>    /* needed to resolve size_t use for strcpy's */ 
/*  end of stuff needed from misc.h  */ 
#endif
...
typedef Bool (*LS_InitProcPtr)(int index, ScreenPtr pScreen, int argc,
	char **argv, char *devName, pointer privArg);
...
typedef struct _LS_Library {
    ...
    LS_InitProcPtr 	Proc;	/* pointer to that procedure       */
...
#define LD_SERVER_PATH_COMP	"X11" /* subdirectory to search first */
...
#define Strncmp(dst, src, n) \
    ((dst == (char *)NULL || src == (char *)NULL) ? 1 : strncmp(dst, src, n))
