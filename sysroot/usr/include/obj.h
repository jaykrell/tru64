 ...
 * |-----------------------------------------------------------|
 * $ */
/* $Header: /share/buffer/build/rcs.dsk1/os/src/usr/include/alpha/obj.h,v 1.1.25.1 2001/09/11 15:02:49 Randy_Lowell Exp $ */
...
/*
 * A write-object identifier.  Used by the obj_create_* routines.
 */
struct wobj;

/*
 * Identifiers for the symbol table sections.  Used by obj_create_addsymscn().
 */
typedef enum {
    WOBJ_SYM_LINE,
    WOBJ_SYM_DNR,
...
/*
 * Identifiers for miscellaneous pieces of information.
 * Used by obj_create_addinfo().
 */
typedef enum {
    WOBJ_INFO_ILINEMAX,  	/* value for HDRR.ilineMax (total number */
                          	/*   of expanded line number entries).   */
				/*   Uses 'val' field.                   */
    WOBJ_INFO_EXTBSS
} wobj_info_type_t;

/*
 * Type for miscellaneous information added with obj_create_addinfo().
 */
typedef union {
    unsigned long	val;
    const void *	ptr;
...
#define OBJ_FAIL	-1
#define MLD_ADDRESS_NIL -3

/*
 * Constants returned by obj_next_next_scnno().
 */
#define WOBJ_SCNNO_SYMTAB	-1
#define WOBJ_SCNNO_DONE  	-2
...
#define LD_NO_SYM	0x1	/* Don't read symbol table. */
#define LD_SYM_ONLY	0x2 	/* Read only symbol table. */
#define LD_FORCE	0x4	/* Ignore mismatching versions. */
#define LD_REPORT	0x8	/* Return control to caller when	
				   an error is encountered during
				   archive processing. */
...
typedef struct obj {
	...
	/* o_data_base_address is used by dbx when debugging loadable device
	 * drivers.  A loadable driver is an OMAGIC file.  When it is mapped
	 * into memory, the text and data+bss regions are mapped to two
	 * separate base addresses.  This field is used to calculated addresses
	 * of data and bss symbols.
	 */

	unsigned long	o_data_base_address;

	struct cmrlc_info *o_cmrlc_info;     /* info for compact relocations */
    long free_required;				     /* flag to indicate raw data is
                                          * malloc'ed and must be free'ed,
                                          * not munmap'ed.
                                          */
} OBJ, *pOBJ;
...
#define obj_procedure_base(obj)	((obj)->o_procedure_base)
...
#define obj_data_base_address(obj) ((obj)->o_data_base_address)
#define obj_set_data_base_address(obj,x) (obj_data_base_address(obj) = (unsigned long)(x))
