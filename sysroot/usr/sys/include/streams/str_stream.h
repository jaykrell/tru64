 ...
 */
/*
 * @(#)$RCSfile: str_stream.h,v $ $Revision: 4.2.94.2 $ (DEC) $Date: 2001/03/27 13:22:18 $
 */
/*
...
struct sqe_s {
	...
	uint			sq_flags;
	ushort                  sq_in_srv;      /* protected by sq_flags_lock. */
	ushort			sq_cpuno;
	...
	void *			sq_arg0;
	void *			sq_arg1;
	decl_simple_lock_data(, sq_flags_lock)
};
...
/* SQ flags */
#define	SQ_IS_HEAD	0x00000001	/* This indicates it's an sqh */
#define	SQ_INUSE	0x00000002	/* Used in both SQEs and SQHs */
					/*  SQEs:  the SQ_INUSE flag         */
					/*   blocks other service procedures */
					/*   for the same queue from being   */
					/*   qenabled while we are running   */
					/*   (i.e., from the qenable() until */
					/*   the csq_lateral()).             */
#define	SQ_HOLD		0x00000004	/* Waiting to become owner of an sqh */
#define	SQ_QUEUED	0x00000008	/* sqe is queued to an sqh */
#define	SQ_IS_TIMEOUT	0x00000010	/* Doesn't seem to be used */
#define	SQ_ON_RUNQ	0x00000020	/* sqe is on the streams_runq */
#define	SQ_IN_SRV	0x00000040	/* Running service procedure */
#define	SQ_IS_CLOSING	0x00000080	/* don't let runq_run run us */
#define	SQ_IS_BLOCKED	0x00000100	/* Waiting for wakeup */
...
/*
 * Open Synchronization Packet
 */
struct osp_s {
	struct osp_s * 	osp_next_dev;	/* next osp in dev hash list 	*/
	struct osp_s * 	osp_next_th;	/* next osp in thread hash list */
	struct thread *	osp_th;		/* ptr to open thread		*/
	struct sth_s *  osp_sth;	/* ptr to stream head		*/
	caddr_t		osp_chan;	/* sleep channel		*/
	dev_t		osp_dev;	/* dev_t which is being opened 	*/
};
...
struct sth_s {
	...
	OSRQ		sth_open_osrq;	/* open request queue */
 ...
 * F_STH_ISAPTM		- stream is a master pty
 * F_STH_FATTACH	- stream is fattached
 *
 * F_STH_CLOSED		- stream has been closed, and should be freed
 * F_STH_CLOSING	- actively on the way down
 * F_STH_TILDE		- write ^A to output stream (used for benchmarking)
 * F_STH_HASHED		- sth_dev hashed with original dev
 *
 ...
 * RWH_ERROR_FLAGS	- same as RWHL except don't consider LINKED error
 */
...
#define	F_STH_ISAPTM		0x0800
#define	F_STH_FATTACH		0x1000

#define	F_STH_CLOSED		0x4000
#define	F_STH_CLOSING		0x8000
#define F_STH_HASHED		0x10000
#define F_STH_TILDE 		0x200000
...
#define	WHL_ERROR_FLAGS		(F_STH_WRITE_ERROR | F_STH_HANGUP \
				| F_STH_LINKED | F_STH_CLOSED)
#define	RWH_ERROR_FLAGS		(F_STH_READ_ERROR | F_STH_WRITE_ERROR \
				| F_STH_HANGUP | F_STH_CLOSED)
#define	RH_ERROR_FLAGS		(F_STH_READ_ERROR  \
				| F_STH_HANGUP | F_STH_CLOSED)
#define	H_ERROR_FLAGS		( F_STH_HANGUP | F_STH_CLOSED)
...
#define	READ_PROTO_USRMASK		(RPROTNORM | RPROTDAT | RPROTDIS )
...
udecl_simple_lock_info(extern, sqh_lockinfo)

#define	sqh_init(sqh)	do {				\
	(sqh)->sqh_next = (sqh)->sqh_prev = (SQP)(sqh);	\
	(sqh)->sqh_flags = SQ_IS_HEAD;			\
	simple_lock_setup(&(sqh)->sqh_lock, sqh_lockinfo);\
	(sqh)->sqh_parent = &sq_sqh;			\
	(sqh)->sqh_owner = nilp(SQ);			\
	(sqh)->sqh_refcnt = 0;				\
} while (0)

udecl_simple_lock_info(extern, sq_flags_lockinfo)

#define	sq_init(sq)	do {				\
	(sq)->sq_flags = 0;				\
	(sq)->sq_in_srv = 0;				\
	simple_lock_setup(&(sq)->sq_flags_lock, sq_flags_lockinfo);\
	/* Other fields set later */			\
} while (0)

#define sq_end(sq)     do {				\
	simple_lock_terminate(&(sq)->sq_flags_lock);	\
} while (0)
