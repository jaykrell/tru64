 ...
 * HISTORY
 *
 *	@(#)$RCSfile: dli_kernel.h,v $ $Revision: 1.1.18.1 $ (DEC) $Date: 2000/09/25 22:40:22 $
 */
...
#define	DLI_FILTER_ONRING	0x00010
    ...
    u_long		fltr_drops;		/* packets dropped */
};
...
#define	DLIF_STARTED	(DLIF_UP|DLIF_RUNNING)
...
#if NETSYNC_LOCK
    lock_data_t		dlif_lock;
    /*
     * Note that dlif_owned is protected the IFQUEUE_LOCK in dlif_rcvq.
     */
    struct ifqueue	dlif_rcvq;		/* serialization queue */
    u_long		dlif_owned;		/* is the queue owned? */
#endif
    struct dli_filter	*dlif_netware_filter;	/* brain-dead NW filter */
};
...
#define	dlif_ifmtu	dlif_ifp->if_mediamtu

#define	dlif_output(dlif, m, sa) \
	((*(dlif)->dlif_ifp->if_output)((dlif)->dlif_ifp, m, sa, NULL, NULL))
...
#define	DLI_FILTER_EQUAL(f1, f2) \
	(DLI_FILTER_LLC_MATCH(f1, &(f2)->fltr_llcdata) || \
	 DLI_FILTER_LLC_MATCH(f2, &(f1)->fltr_llcdata))
...
#if NETSYNC_LOCK
extern struct lockinfo *dli_dlif_li;
extern struct lockinfo *dli_input_li;
extern lock_data_t dli_input_lock;
_BEGIN_CPLUSPLUS
extern void dli_lock_input __((struct socket *so));
_END_CPLUSPLUS

#define	dli_funnel	NULL

#define	DLI_DLIF_LOCKINIT(dlif)	lock_setup(&(dlif)->dlif_lock, dli_dlif_li, TRUE)
#define	DLI_DLIF_LOCK(dlif)	lock_write(&dlif->dlif_lock)
#define	DLI_DLIF_UNLOCK(dlif)	lock_done(&dlif->dlif_lock)

#define	DLI_INPUT_LOCKINIT()	lock_setup(&dli_input_lock, dli_input_li, TRUE)
#define	DLI_INPUT_LOCK()	lock_read(&dli_input_lock)
#define	DLI_INPUT_LOCKWRITE()	lock_write(&dli_input_lock)
#define	DLI_INPUT_LOCK2(so)	dli_lock_input(so)
#define	DLI_INPUT_UNLOCK()	lock_done(&dli_input_lock)
#else
/* Avoid scoping problems */
typedef union {
	struct domain_funnel	*p1;
} proto_dli_union_t;
_BEGIN_CPLUSPLUS
extern void dli_funnel __((struct domain_funnel *));
_END_CPLUSPLUS

#define	DLI_DLIF_LOCKINIT(dlif)	0
#define	DLI_DLIF_LOCK(dlif)	0
#define	DLI_DLIF_UNLOCK(dlif)	0

#define	DLI_INPUT_LOCKINIT()	0
#define	DLI_INPUT_LOCK()	0
#define	DLI_INPUT_LOCKWRITE()	0
#define	DLI_INPUT_LOCK2(so)	0
#define	DLI_INPUT_UNLOCK()	0
#endif
...
const extern struct dlimgt_routines *dlimgtsw;
...
extern int dli_bind __((struct dli_pcb **, struct sockaddr_dl *));
extern int dli_unbind __((struct dli_pcb **));
extern int dli_rcv_user __((DLI_FILTER_MATCH_ARGS));

/* dli_filter.c */

extern int dli_filter_create __((struct dli_ifnet *, struct dli_filter **, unsigned, const u_char *));
extern int dli_filter_add __((struct dli_ifnet *, const struct sockaddr_dl *, struct dli_filter **));
extern int dli_filter_add_ether __((struct dli_ifnet *, const struct sockaddr_edl *, int, struct dli_filter **));
extern int dli_filter_add_sap __((struct dli_ifnet *, const struct sockaddr_802 *, int, struct dli_filter **));
extern int dli_filter_add_snapsap __((struct dli_ifnet *, const struct sockaddr_802 *, int, struct dli_filter **));
extern int dli_filter_insert __((struct dli_ifnet *, struct dli_filter *));
extern void dli_filter_destroy __((struct dli_ifnet *, struct dli_filter **));
extern void dli_filter_input __((struct dli_recv *, struct mbuf *));

extern int dli_filter_add_gsap __((struct dli_ifnet *, struct dli_filter *, u_int));
extern int dli_filter_test_gsap __((struct dli_filter *, u_int));
extern int dli_filter_remove_gsap __((struct dli_ifnet *, struct dli_filter *, u_int));
extern void dli_filter_disable_gsap __((struct dli_ifnet *, struct dli_filter *, u_int));
extern void dli_filter_disable_all_gsaps __((struct dli_ifnet *, struct dli_filter *));
extern void dli_filter_by_gsap __((struct dli_recv *, struct mbuf *));

/* dli_init.c */

extern void dli_dlif_add __((struct ifnet *));
extern void dli_init __((void));
extern void dli_dlif_start __((struct dli_ifnet *));
extern void dli_dlif_stop __((struct ifnet *));
extern void dli_ctlinput __((int, struct sockaddr *, caddr_t));

/* dli_input.c */

extern void dli_intr __((void));
extern void dli_input __((struct dli_ifnet *, struct mbuf *));
extern int dli_rcv_addr_filter __((DLI_FILTER_MATCH_ARGS));
extern int dli_rcv_loopback __((DLI_FILTER_MATCH_ARGS));
extern int dli_rcv_moprc __((DLI_FILTER_MATCH_ARGS));
extern int dli_rcv_nullsap __((DLI_FILTER_MATCH_ARGS));
extern int dli_rcv_snapsap __((DLI_FILTER_MATCH_ARGS));

/* dli_mopctrs.c */

extern struct mbuf *dli_get_ether_ctrs __((struct dli_ifnet *, u_int));
extern struct mbuf *dli_get_fddi_ctrs __((struct dli_ifnet *, u_int));
extern void dli_snd_mopctrs __((struct dli_ifnet *, struct dli_filter *, u_int, const union dli_addr *));

/* dli_output.c */

extern int dli_send __((struct dli_pcb *, struct mbuf *m, struct sockaddr_dl *));
extern int dli_add_header __((struct dli_ifnet *, struct mbuf **m, const union dli_llcdata *, int));
extern int dli_output __((struct dli_ifnet *, struct dli_filter *, struct mbuf *, const union dli_addr *));

/* dli_sockopt.c */

extern int dli_getopt __((struct dli_pcb *, u_char *, int *, int));
extern int dli_setopt __((struct dli_pcb *, u_char *, int, int));

/* dli_subr.c */

extern int dli_llc_responder __((struct dli_recv *, struct mbuf *, struct dli_filter *));
extern u_int dli_addr_hash __((const union dli_addr *));
extern struct dli_mcast *dli_addr_lookup __((struct dli_ifnet *, const struct dli_filter *filter, const union dli_addr *, const struct dli_fagamap *));
extern void dli_addrset_init __((struct dli_ifnet *, const struct dli_filter *, struct dli_addrset *));
extern int dli_addrset_add __((struct dli_ifnet *, const struct dli_filter *, struct dli_addrset *, const union dli_addr *));
extern int dli_addrset_test __((struct dli_ifnet *, const struct dli_filter *, struct dli_addrset *, const union dli_addr *));
extern void dli_addrset_remove_int __((struct dli_ifnet *, struct dli_addrset *, struct dli_mcast *, const struct dli_fagamap *));
extern int dli_addrset_remove __((struct dli_ifnet *, const struct dli_filter *, struct dli_addrset *, const union dli_addr *));
extern void dli_addrset_destroy __((struct dli_ifnet *, const struct dli_filter *, struct dli_addrset *));
extern struct dli_ifnet *dli_devid2dlif __((struct dli_devid *));
#ifndef dli_ifp2dlif
extern struct dli_ifnet *dli_ifp2dlif __((struct ifnet *));
#endif
extern struct mbuf *dli_trim_etherpad __((struct mbuf *));
extern const struct dli_fagamap *dli_addr_remap __((struct dli_ifnet *, const struct dli_filter *, const union dli_addr *, int));
extern int dli_rcv_and_drop __((DLI_FILTER_MATCH_ARGS));

/* dli_timers.c */

extern void dli_slowtimo __((void));
extern void dli_snd_sysid __((struct dli_ifnet *, struct dli_filter *, int, const union dli_addr *));

/* dli_usrreq.c */

extern int dli_usrreq __((struct socket *so, int, struct mbuf *, struct mbuf *, struct mbuf *));
extern int dli_ctloutput __((int, struct socket *, int, int, struct mbuf **));
extern int dli_control __((struct dli_pcb *, unsigned int, caddr_t, struct dli_ifnet *));
extern int dli_validate_addr __((struct mbuf *));

/*
 * these functions are present only if the DNxDLI (DLIMGT) subset is installed
 */
extern int dlimgt_openport __((struct dli_ifnet *, struct dli_filter *));
extern void dlimgt_closeport __((struct dli_ifnet *, struct dli_filter *));
extern int dlimgt_prco_pif __((int, struct socket *, int, int, struct mbuf **));
extern int dlimgt_control __((struct dli_pcb *, u_int, caddr_t, struct dli_ifnet *, int *));
extern void dlimgt_sysid __((struct dli_ifnet *, int, struct mbuf *));
extern int dlimgt_log_unrecog_pdu __((struct dli_recv *, struct mbuf *));
extern struct mbuf *dlimgt_get_dlctrs __((struct dli_ifnet *, int));
_END_CPLUSPLUS

struct dlimgt_routines {
    /* these function pointers are nonNULL only if the NETMAN subset is installed */
   int (*mgt_openport) __((struct dli_ifnet *, struct dli_filter *));
   void (*mgt_closeport) __((struct dli_ifnet *, struct dli_filter *));
   int (*mgt_control) __((struct dli_pcb *, u_int, caddr_t, struct dli_ifnet *, int *));
   int (*mgt_prco_pif) __((int, struct socket *, int, int, struct mbuf **));
   void (*mgt_sysid_extra) __((struct dli_ifnet *, int, struct mbuf *));
   int (*mgt_log_unrecog_pdu) __((struct dli_recv *, struct mbuf *));
   struct mbuf *(*mgt_get_dlctrs) __((struct dli_ifnet *, int));
};
#endif /* _KERNEL */
