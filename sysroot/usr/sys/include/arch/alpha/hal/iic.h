...
extern int	     write_iic_eeprom(unsigned char device, 
				      unsigned char addr, 
				      unsigned char *data, 
				      unsigned char size);
extern unsigned char *read_iic_eeprom(unsigned char device, 
				      unsigned char addr, 
				      unsigned char size);
extern void	     read_entire_iic_eeprom(unsigned char device);

extern void wait_for_iic_free(void);
extern void wait_to_be_addressed(void);
extern void wait_for_pin(void);
extern int  check_lrb_on_read(unsigned int dev, 
			      unsigned int addr);
extern int  check_lrb_on_write(unsigned int dev, 
			       unsigned int addr);
extern void init_iic(void);
extern int ocp_led_off(enum ocp_led led);
extern int ocp_led_on(enum ocp_led led);
extern unsigned char ocp_read(enum ocp_rd_dev device);
extern unsigned char ocp_get_state(unsigned char address);


/*
 * errorlog is a union of two structs defined in kn_430.h,
...
extern int log_iic_error(
			 unsigned char descriptor,
			 unsigned char device,
			 unsigned char num_errors,
			 union iic_errorlog errorlog
			 );
