 ...
 */
/*
 *	@(#)$RCSfile: vm_param.h,v $ $Revision: 4.2.17.1 $ (DEC) $Date: 2000/03/16 23:02:46 $
 */
/*
...
/*
 * Interfaces to obtain memory-related parameters
 */
extern kern_return_t	vm_get_pa_span(vm_offset_t *begin, vm_offset_t *end);
extern kern_return_t	vm_get_memsize(unsigned long flags, vm_size_t *size);

/*
 * Defines for flags in vm_get_memsize ()
 */
#define VM_MEMSIZE	 0x01	/* size of memory; if !VM_LOCAL, then all of memory */
#define VM_LOCAL_MEMSIZE 0x02	/* size of memory, local/closest to currently running CPU */
