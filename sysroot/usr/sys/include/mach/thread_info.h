 ...
 */
/*	
 *	@(#)$RCSfile: thread_info.h,v $ $Revision: 4.2.23.1 $ (DEC) $Date: 2000/11/09 22:37:38 $
 */ 
/*
 */
/*
...
#define TH_BOUND_PROCESSOR	0x1
#define TH_BOUND_PSET		0x2
...
struct thread_sched_info {
	...
	int		cpu;		/* current cpu # */
	int		bound;		/* bound to cpu/pset ? */
	int		pset;		/* pset number */
};
 ...
 *	Currently defined information.
 */
#define THREAD_SIGNAL_INFO	4		/* signal information */

struct thread_signal_info {
	int		cursig;		/* current signal */
...
#define THREAD_SIGNAL_INFO_COUNT	\
	((sizeof(thread_signal_info_data_t) + sizeof(long) - 1) / sizeof(long))

#define THREAD_NUMA_INFO	5

struct thread_numa_info {
	pid_t		tninfo_pid;		/* PID for thread */
	radid_t		tninfo_radid;		/* Home RAD for thread */
	u_long		tninfo_attrib;		/* thread NUMA attributes */
	long		tninfo_pad[5];		/* future development */
};

/* tninfo_attrib mask bits */
#define TNINFO_RAD_BIND		NPINFO_RAD_BIND       /* is bound to a RAD */
#define TNINFO_RAD_ATTACH	NPINFO_RAD_ATTACH     /* is attached to a RAD */
#define TNINFO_RAD_NO_INHERIT	NPINFO_RAD_NO_INHERIT /* child no inherit RAD */

typedef struct thread_numa_info	thread_numa_info_data_t;
typedef struct thread_numa_info	*thread_numa_info_t;
#define THREAD_NUMA_INFO_COUNT	\
	((sizeof(thread_numa_info_data_t) + sizeof(long) - 1) / sizeof(long))
