 ...
 */
/*
 * @(#)$RCSfile: sim_common.h,v $ $Revision: 1.1.26.1 $ (DEC) $Date: 2000/03/14 17:21:11 $
 */
#ifndef _SIM_COMM_
#define _SIM_COMM_
#ifdef __cplusplus
...
/*
 * Function prototypes for sim_common functions
 */

char            *sc_alloc(int size);
void            sc_free(char *c, u_int size);
...
#define SC_HBA_TARG_ENABLE(ccb, sc, nexus)                                 \
    ((sc)->hba_targ_enable)(ccb, sc, nexus)
 ...
 */
#define SC_WS_REMOVE(sws) \
                if (0 == (sws->flags & SZ_IN_USE)){      \
                        panic("sc ws remove: SZ_IN_USE NOT set\n");} \
                sws->flags &= ~SZ_IN_USE;   \
                remque((void *)(sws));

#define SC_WS_INSERT(sws, where) \
                if (sws->flags & SZ_IN_USE){    \
                        panic("sc ws insert: SZ_IN_USE ALREADY set\n"); }\
                sws->flags |= SZ_IN_USE;                                \
                insque((void *)(sws), (void *)(where));
...
/* 
 * Macro Name: SC_GET_CCB
 *	Calculate the CCB pointer from the associated
 *	SIM_WS pointer.  This macro depends on the
 *	placement of the SIM_WS within the DEC CAM
 *	packet.
 *	
 * Arguments:
 *	sws	Pointer to the SIM_WS structure.
 */
#define SC_GET_CCB(sws) \
   	(CCB_HEADER *)(((U_WORD)(sws)) + ((U_WORD)sizeof(SIM_WS)))
