...
/*	
 *	@(#)$RCSfile: cdrom.h,v $ $Revision: 1.1.15.1 $ (DEC) $Date: 2000/03/16 17:40:30 $
 */ 
/*
 */
/*
...
#define CDROM_FOUR_CHAN_AUDIO	0x08	/* 0 = 2 Channel, 1 = 4 Channel.   */
...
/*
 * Define Format of Entrys Table of Contents for a single entry.
 */
struct toc_single_entry {
	struct cd_toc_header	toc_h;	/* TOC header			*/
	struct cd_toc_entry	toc_td;	/* TOC track descriptor		*/
};
...
/*
 * Define Format of Table of Contents data for SCSI-3 format 0100b.
 */
struct cd_toc_s3_data {
      struct cd_toc_header header;		/* Header area */
      u_char	te3_sessnum	: 8;	/* Session Number.		*/
      u_char			: 8;	/* Reserved.			*/
      u_char	te3_first_trk_sess;	/* The first trk num in this session.*/
      u_char			: 8;	/* Reserved.			*/
      union cd_address te3_absaddr;	/* Absolute CD-ROM Address.
					   of first block in last session*/
};

/*
 * Define Format of Table of Contents data for SCSI-2 format 01b.
 * Same format is used for ATAPI READ TOC format 01b or 0100b data
 */
struct cd_toc_s2_data {
      struct cd_toc_header header;		/* Header area */
      u_char			: 8;	/* Reserved.			*/
      u_char	te2_control	: 4;	/* Control field (attributes).	*/
      u_char	te2_addr_type	: 4;	/* Address type information.	*/
      u_char	te2_first_trk_sess;	/* The first trk num in last session.*/
      u_char			: 8;	/* Reserved.			*/
      union cd_address te2_absaddr;	/* Absolute CD-ROM Address.
					   of first block in last session*/
};
...
/*
 * Structure for CDROM_GET_SESSION_INFO IOCTL Command.
 * (for multi-session CD support)
 */
struct cd_session_info {
	u_int	sessinf_block_size;	/* Block size that the following value
					   is in terms of. */
	u_int	sessinf_first_block_last_sess;/* Starting block number of last session.	*/
};
 ...
 */
#define CDROM_DEFAULT_DENSITY	0x00	/* Default density code.	*/
#define CDROM_USER_DATA_ONLY	0x01	/* User data only (2048 bytes)	*/
#define CDROM_USER_AUX_DATA	0x02	/* User data + aux (2336 bytes)	*/
#define CDROM_TAG_AUX_DATA	0x03	/* Tag + user + aux (2340 bytes)*/
#define CDROM_AUDIO_INFO_ONLY	0x04	/* Audio information only.	*/
#define CDROM_RESERVED_CODES	0x7f	/* 0x05 - 0x7f = reserved codes	*/
#define CDROM_VENDOR_UNIQUE	0x80	/* 0x80 - 0xff = Vendor unique.	*/

/*
 * Toshiba (RRD43/44) Vendor Unique Density Codes:
 */
#define TOSHIBA_XA_SUPPORT	0x81	/* CD-ROM XA support mode.	*/
#define TOSHIBA_CD_DA_XFER	0x82	/* CD-DA SCSI transfer mode.	*/
#define TOSHIBA_MODE1_XA_FORM1	0x83	/* Mode-1 & XA Form-1 support.	*/
...
#define CDROM_GET_SESSION_INFO	\
	_IOWR('p', 41, struct cd_session_info)  /* Get Session Info.	*/
#define CDROM_USE_NATIVE_BLKSIZE	\
	_IOWR('p', 42, int)  			/* Use Native Blocksize	*/
