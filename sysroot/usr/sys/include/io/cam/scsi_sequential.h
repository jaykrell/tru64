 ...
 */
/*
 * @(#)$RCSfile: scsi_sequential.h,v $ $Revision: 1.1.18.5 $ (DEC) $Date: 1998/12/02 13:54:17 $
 */
#ifndef _SCSI_SEQ_
#define _SCSI_SEQ_
...
typedef struct seq_locate_cdb10 {
	u_char	opcode;			/* 0x2B				*/
	u_char	immed		:1,	/* Immediate bit 		*/
		cp		:1,	/* Change partition (unused)	*/
		bt		:1,	/* Block Address Type		*/
				:5;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char	blk_addr3	:8;	/* reserved			*/
	u_char	blk_addr2	:8;	/* reserved			*/
	u_char	blk_addr1	:8;	/* reserved			*/
	u_char	blk_addr0	:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char	part		:8;	/* Partition (unused)		*/
	u_char	control;		/* Control byte			*/
}SEQ_LOCATE_CDB10;

/*
 * LOCATE opcode define
 */
#define SEQ_LOCATE_OP	0x2B

/*
 * Take a logical address block and place it into a 10 byte locate cdb 
 * SEQ_COUNT_TO_LOCATECDB10
 */

#define SEQ_COUNT_TO_LOCATECDB10(count,cdb); { \
	union { \
		unsigned char c[4]; \
		U32		 l; \
	}tmp; \
	tmp.l = (count); \
	((SEQ_LOCATE_CDB10 *)(cdb))->blk_addr0 = tmp.c[0]; \
	((SEQ_LOCATE_CDB10 *)(cdb))->blk_addr1 = tmp.c[1]; \
	((SEQ_LOCATE_CDB10 *)(cdb))->blk_addr2 = tmp.c[2]; \
	((SEQ_LOCATE_CDB10 *)(cdb))->blk_addr3 = tmp.c[3]; \
}
...
typedef struct seq_block_limits_data {
	u_char	granularity	:5,	/* block size granularity	*/
				:3;	/* Reserved			*/
 ...
 * Read position
 */
typedef struct seq_read_pos_cdb10 {
	u_char	opcode;		/* 0x34					*/
	u_char	bt		:1,	/* Block Address Type		*/
		l_fmt		:1,	/* Long format			*/
		tclp		:1,	/* Tot Current Logical Position	*/
				:5;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
 ...
 */
#define	SEQ_READ_POS_OP		0x34

/* 
 * READ POSITION data format, short form
 */
typedef struct seq_read_pos_short_data {
	u_char			:1,	/* reserved			*/
	...
	u_char	part_num;		/* Partition number		*/

	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/

	u_char	first_blk3;		/* MSB First block location	*/
	u_char	first_blk2;		/*     First block location	*/
	u_char	first_blk1;		/*     First block location	*/
	u_char	first_blk0;		/* LSB First block location	*/

	u_char	last_blk3;		/* MSB Last block location	*/
	u_char	last_blk2;		/*     Last block location	*/
	u_char	last_blk1;		/*     Last block location	*/
	u_char	last_blk0;		/* LSB Last block location	*/

	u_char			:8;	/* reserved			*/

	u_char	num_blks2;		/* MSB Number of blocks in buffer */
	u_char	num_blks1;		/*     Number of blocks in buffer */
	u_char	num_blks0;		/* LSB Number of blocks in buffer */

	u_char	num_bytes3;		/* MSB Number of bytes in buffer */
	u_char	num_bytes2;		/*     Number of bytes in buffer */
	u_char	num_bytes1;		/*     Number of bytes in buffer */
	u_char	num_bytes0;		/* LSB Number of bytes in buffer */
}SEQ_READ_POS_SHORT_DATA;

/* 
 * READ POSITION data format, long form
 */
typedef struct seq_read_pos_long_data {
	u_char			:2,	/* reserved			*/
		bpu		:1,	/* Block position unknown	*/
	...
	u_char	part_num3;		/* MSB Partition number		*/
	u_char	part_num2;		/*     Partition number		*/
	u_char	part_num1;		/*     Partition number		*/
	u_char	part_num0;		/* LSB Partition number		*/

	u_char	blk_num7;		/* MSB Block number		*/
	u_char	blk_num6;		/*     Block number		*/
	u_char	blk_num5;		/*     Block number		*/
	u_char	blk_num4;		/*     Block number		*/
	u_char	blk_num3;		/*     Block number		*/
	u_char	blk_num2;		/*     Block number		*/
	u_char	blk_num1;		/*     Block number		*/
	u_char	blk_num0;		/* LSB Block number		*/

	u_char	file_num7;		/* MSB File number		*/
	u_char	file_num6;		/*     File number		*/
	u_char	file_num5;		/*     File number		*/
	u_char	file_num4;		/*     File number		*/
	u_char	file_num3;		/*     File number		*/
	u_char	file_num2;		/*     File number		*/
	u_char	file_num1;		/*     File number		*/
	u_char	file_num0;		/* LSB File number		*/

	u_char	set_num7;		/* MSB Set number		*/
	u_char	set_num6;		/*     Set number		*/
	u_char	set_num5;		/*     Set number		*/
	u_char	set_num4;		/*     Set number		*/
	u_char	set_num3;		/*     Set number		*/
	u_char	set_num2;		/*     Set number		*/
	u_char	set_num1;		/*     Set number		*/
	u_char	set_num0;		/* LSB Set number		*/
}SEQ_READ_POS_LONG_DATA;
...
typedef struct seq_release_cdb6 {
	u_char	opcode;		/* 0x17					*/
	u_char	extent		:1,	/* Extent release		*/
		third_dev_id	:3,	/* Third party dev id		*/
		third_pat	:1,	/* Third party bit		*/
		lun		:3;	/* Logical unit number 		*/
	u_char	res_id		:8;	/* Reservation identification	*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char	control;		/* Control byte			*/
}SEQ_RELEASE_CDB6;
...
/*
 * Release unit command cdb 10 byte
 */
typedef struct seq_release_cdb10 {
	u_char	opcode;		/* 0x57					*/
	u_char	extent		:1,	/* Extent release		*/
		long_id		:1,	/* Long ID			*/
				:2,	/* reserved			*/
		third_pat	:1,	/* Third party bit		*/
				:3;	/* reserved			*/
	u_char	res_id		:8;	/* Reservation identification	*/
	u_char	third_dev_id	:8;	/* Third party dev id		*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char	list_len1	:8;	/* MSB Parameter list length	*/
	u_char	list_len0	:8;	/* LSB Parameter list length	*/
	u_char	control;		/* Control byte			*/
}SEQ_RELEASE_CDB10;
 ...
 */
typedef struct seq_report_density_cdb10 {
	u_char	opcode;		/* 0x44					*/
	u_char	media		:1,	/* Media			*/
				:7;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char	all_len1;		/* MSB allocation length	*/
	u_char	all_len0;		/* LSB allocation length	*/
	u_char	control;		/* Control byte			*/
}SEQ_REPORT_DENSITY_CDB;

#define	SEQ_REPORT_DENSITY_OP	0x44

typedef struct seq_den_head {
	u_char	avail_den_len1;		/* MSB Avail Density length	*/
	u_char	avail_den_len0;		/* LSB Avail Density length	*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
}SEQ_DEN_HEAD;

typedef struct seq_den_data {
	u_char	prim_code;		/* Primary density code		*/
	u_char	sec_code;		/* Secondary density code	*/
	u_char			:5,	/* reserved			*/
		deflt		:1,	/* default density		*/
		dup		:1,	/* duplicate			*/
		wrt_ok		:1;	/* writeable			*/
	u_char			:8;	/* reserved			*/
	u_char			:8;	/* reserved			*/
	u_char	bits_mm2;		/* MSB bits/mm			*/
	u_char	bits_mm1;		/*     bits/mm			*/
	u_char	bits_mm0;		/* LSB bits/mm			*/
	u_char	media_width1;		/* MSB Media width		*/
	u_char	media_width0;		/* LSB Media width		*/
	u_char	tracks1;		/* MSB Tracks			*/
	u_char	tracks0;		/* LSB Tracks			*/
	u_char	capacity3;		/* MSB Capacity			*/
	u_char	capacity2;		/*     Capacity			*/
	u_char	capacity1;		/*     Capacity			*/
	u_char	capacity0;		/* LSB Capacity			*/
	u_char	organization[8];	/* Assigning organization	*/
	u_char	density_name[8];	/* Density name			*/
	u_char	description[20];	/* Density description		*/
}SEQ_DEN_DATA;

typedef struct seq_report_den_data {
	SEQ_DEN_HEAD		den_head;	/* The header		*/
	SEQ_DEN_DATA		den_data;	/* The data		*/
}SEQ_REPORT_DEN_DATA;

/*
 * Put the allocation length into the REPORT DENSITY SUPPORT CDB.
 */

#define SEQ_LEN_TO_REP_DENS_CDB(length,cdb); { \
	union { \
		unsigned char c[4]; \
		U16		 l; \
	}tmp; \
	tmp.l = (length); \
	((SEQ_REPORT_DENSITY_CDB *)(cdb))->all_len0 = tmp.c[0]; \
	((SEQ_REPORT_DENSITY_CDB *)(cdb))->all_len1 = tmp.c[1]; \
}
...
typedef struct seq_reserve_cdb6 {
	u_char	opcode;		/* 0x16					*/
	u_char	extent		:1,	/* Extent reservation		*/
		third_dev_id	:3,	/* Third party dev id		*/
		third_pat	:1,	/* Third party bit		*/
		lun		:3;	/* Logical unit number 		*/
	u_char	res_id		:8;	/* Reservation identification	*/
	u_char	list_len1	:8;	/* MSB Extent list length	*/
	u_char	list_len0	:8;	/* LSB Extent list length	*/
	u_char	control;		/* Control byte			*/
}SEQ_RESERVE_CDB6;
...
/* 
 * Reserve command cdb 10 byte 
 */
typedef struct seq_reserve_cdb10 {
	u_char	opcode;		/* 0x56					*/
	u_char	extent		:1,	/* Extent reservation		*/
		long_id		:1,	/* Long ID			*/
				:2,	/* reserved			*/
...
typedef struct seq_verify_cdb6 {
	u_char	opcode;			/* 0x13  Operation Code.	[0] */
...
#define SEQ_PGM_DATA_COMPR	0x0F	/* Data compression page	*/
...
#define SEQ_81630_BPI	0x1A		/* 0x1A - TZ88 tape mfm		*/
 ...
 */
typedef struct seq_data_compr_pg {
	SEQ_MODE_PG_HD	pg_head;	/* Page header			*/
	u_char			:6,	/* Reserved bits		*/
		dcc		:1,	/* Data compression capable	*/
		dce		:1;	/* Data compression enable	*/
	u_char			:5,	/* Reserved bits		*/
		red		:2,	/* Report Exception on Decompr  */
		dde		:1;	/* Data Decompression Enable	*/
	u_char	compr_alg3;		/* MSB Compression Algorithm	*/
	u_char	compr_alg2;		/*     Compression Algorithm	*/
	u_char	compr_alg1;		/*     Compression Algorithm	*/
	u_char	compr_alg0;		/* LSB Compression Algorithm	*/
	u_char	decompr_alg3;		/* MSB Decompression Algorithm	*/
	u_char	decompr_alg2;		/*     Decompression Algorithm	*/
	u_char	decompr_alg1;		/*     Decompression Algorithm	*/
	u_char	decompr_alg0;		/* LSB Decompression Algorithm	*/
	u_char			:8;	/* Reserved			*/
	u_char			:8;	/* Reserved			*/
	u_char			:8;	/* Reserved			*/
	u_char			:8;	/* Reserved			*/
...
typedef struct seq_part1_pg {
	...
	u_char	part_units	:4,	/* Partition units		*/
				:4;	/* Reserved			*/
	u_char			:8;	/* Reserved			*/
...
#define SEQ_PSUM_PBYTES	03	/* Measure is in 10**part_units bytes	*/
