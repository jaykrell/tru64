 ...
 */
/*
 * @(#)$RCSfile: dme_common.h,v $ $Revision: 1.1.17.2 $ (DEC) $Date: 1996/04/30 21:15:26 $
 */
...
decl_simple_lock_info(extern, d3cntrl_lockinfo)
#define D3CTRL_INIT_LOCK( d3p )                          \
{                                                        \
    usimple_lock_setup( (simple_lock_t)&((d3p)->d3_lk_ctrl),\
			d3cntrl_lockinfo );     \
}
...
{                                                        \
    (saveipl) = splbio();                                \
    CAM_SPINLOCK( (simple_lock_t)&((d3p)->d3_lk_ctrl) ); \
}

#define D3CTRL_IPLSMP_UNLOCK( saveipl, d3p )             \
{                                                        \
    CAM_SPINUNLOCK( (simple_lock_t)&((d3p)->d3_lk_ctrl) ); \
    (void)splx((saveipl));                               \
}

#define D3CTRL_SMP_SLEEPUNLOCK( chan, pri, d3p )         \
{                                                        \
    CAM_SLEEP_SPINUNLOCK( chan, pri, (simple_lock_t)&((d3p)->d3_lk_ctrl) );\
}
...
#if defined(__alpha) || defined(__intel__)
