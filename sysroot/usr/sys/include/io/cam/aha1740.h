 ...
 */
/*
 * @(#)$RCSfile: aha1740.h,v $ $Revision: 1.1.19.4 $ (DEC) $Date: 1998/05/11 14:35:49 $
 */
 ...
 */
#define SCSIDEF_ID_MASK         0xf     /* 0-7 valid, 8+ reserved       */
...
#define HS_FRM_NT_DL    0x08    /* Firmware Not Downloaded      */
#define HS_TRG_NT_ASG   0x0a    /* Target not assigned to me    */
...
#define HS_IV_OPC       0x16    /* Invalid opcode               */
#define HS_IV_SLO	0x17	/* Invalid SCSI linking Oper.	*/
#define HS_IV_CB_PAR    0x18    /* Invalid Control Block Param. */
#define HS_DUP_CB       0x19    /* Duplicate Target Control Blk.*/
#define HS_IV_SG        0x1a    /* Invalid S/G list             */
...
typedef struct aha_initsel 
{
    unsigned direction:1;      /* direction of requested data transfer */
    unsigned reserved:3;       /* not used                             */
    unsigned lun:4;            /* LUN of requested host adapter        */
    U8       cdb_2;            /* CDB byte 2 of incoming command       */
    U8       cdb_3;            /* CDB byte 3 of incoming command       */
    U8       cdb_4;            /* CDB byte 4 of incoming command       */
} AHA_INITSEL;

#define AHA_MIN_FIRMREV    'G.2 ' /* Minimum acceptable firmware revision *
...
#define AHA_SOFTC_VERSION 2
  ...
  struct bus *aha_ctlr;		        /* bus adapter reference	   */
 ...
 * entry.
 */
#define AHA_DMA_MAX_COUNT	0x3FE000	/* < 4Mb max per entry	   */
						/* actually (4MB - 8k)     */
...
#define AHA_JOB_VERSION 1
  ...
  dma_handle_t  aha_dma_h[AHA_DMA_MAX_SEGMENTS];
                                        /* place to save dma handles      */
  dma_handle_t	aha_dma_cb_h;		/* dma handle for the cb	  */
...
#define AHA_SPECIAL	2		/* Special job block for bus    *
				 	 * device reset			*/
/*
 * defines for the bus structure to hold fields that
 * used to be stored in the controller struct;
 *      ctlr->xxxxx is now bus->adpt_xxxxx
 */

#define adpt_busfuncs      private[0]   /* reserved for &eisa_funcs */
#define adpt_hid           private[1]   /* Interrup handler ID */
#define adpt_addr          private[2]   /* base I/O slot addr */
#define adpt_addr2         private[3]   /* aha softc struct */
#define adpt_physaddr      private[4]   /* base CSR addr in AHA space */
#define adpt_eisainfo      private[5]   /* adapter specific structure */

/*
 * Locking macros - handle IPL and SMP locks
 */

#define AHA_LOCK_INIT(lock, lockinfo) 		\
