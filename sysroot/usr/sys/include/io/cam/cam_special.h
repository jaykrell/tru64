 ...
 */
/*
 * @(#)$RCSfile: cam_special.h,v $ $Revision: 1.1.13.2 $ (DEC) $Date: 2002/05/30 23:15:23 $
 */
...
typedef	struct special_cmd {
	...
	u_char *spc_cdbp;		/* Pointer to prototype	CDB.	*/
...
typedef	struct special_args {
	...
	int	(*sa_error)(CCB_SCSIIO		*ccb,
			    ALL_REQ_SNS_DATA	*sense);
					/* The error report function.	*/
	U32	(*sa_start)(CCB_HEADER *ccb);
					/* The driver start function.	*/
	int	sa_data_length;		/* Kernel data buffer length.	*/
	caddr_t	sa_data_buffer;		/* Kernel data buffer address.	*/
	u_char *sa_cdb_pointer;		/* Pointer to the CDB buffer.	*/
...
extern void 
	scmn_SpecialInit(void),	
	scmn_AddSpecialCmds(struct special_header *sph), 
	scmn_SpecialCleanup(struct special_args	*sap);
extern int
	scmn_RemoveSpecialCmds(struct special_header *sph),
	scmn_FreeArgsBuffer(struct special_args	*sap),
	scmn_SpecialCmd(struct special_args *sap),
	scmn_DoCmd(struct special_args *sap),
	scmn_AbortCCB(CCB_SCSIIO *ccb),
	scmn_ReleaseSIMQ(CCB_SCSIIO *ccb),
	scmn_TerminateCCB(CCB_SCSIIO *ccb),
	scmn_cdb_length(u_char cmd_code);
