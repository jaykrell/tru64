 ...
 */
/*
 * @(#)$RCSfile: scsi_direct.h,v $ $Revision: 1.1.22.2 $ (DEC) $Date: 2002/01/30 18:10:54 $
 */
#ifndef _SCSI_DIR_
#define _SCSI_DIR_
...
/* 
 * The defect list format codes.
 */
#define DIR_BLOCK_FMT	0x0	/* BLOCK FORMAT				*/
#define DIR_INDEX_FMT	0x4	/* BYTES FROM INDEX FORMAT		*/
#define DIR_SECTOR_FMT	0x5	/* SECTOR FORMAT			*/
...
typedef struct dir_defect_header {
	...
	u_char	def_list_len1;	/* MSB of defect list length		*/
	u_char	def_list_len0;	/* LSB of defect list length		*/
}DIR_DEFECT_HEADER;
...
typedef struct dir_pattern_desc {
	u_char			:5,	/* reserved			*/
		si		:1,	/* Security Initialize		*/
		ip_mod		:2;	/* Init pattern modifier	*/
	u_char	pattern_type;		/* Pattern type			*/
...
typedef struct dir_fmt_para_list_pat {
	...
	DIR_PATTERN_DESC	*pattern_desc;	/* to pattern descriptor */
		/* 
		 * The following are pointers to arrays of defect 
		 * descriptors
		 */
	union {
...
typedef struct dir_cache_lock_cdb10 {
	...
	u_char	reladr	:1,	/* Relative address bit.		*/
		lock	:1,	/* lock or unlock			*/
			:3,	/* 3 bits reserved			*/
		lun	:3;	/* Logical unit number			*/
	u_char	lbn4;		/* MSB of logical block number		*/
	u_char	lbn3;		/*   Middle of logical block number	*/
	u_char	lbn2;		/*   Middle of logical block number	*/
	u_char	lbn1;		/*   Middle of logical block number	*/
...
/*
 * Pre-fetch cdb 10 byte
 */
typedef struct dir_pre_fetch_cdb10 {
        u_char  opcode;         /* 0x34                                 */
        u_char  reladr  :1,     /* Relative address bit.                */
                immed   :1,     /* Immediate                            */
                        :6;     /* 6 bits reserved                      */
...
typedef struct dir_read_cap_cdb10 {
	...
	u_char		:8;	/* Reserved				*/
	u_char		:8;	/* Reserved				*/
	u_char	pmi	:1,	/* Partial medium indicator		*/
			:7;	/* 7 bits reserved			*/
	u_char	control;	/* the control byte			*/
}DIR_READ_CAP_CDB10;
 ...
 * Defect list types:
 */
#define DIR_VENDOR_DEFECTS	0
#define DIR_KNOWN_DEFECTS	1
#define	DIR_NO_DEFECTS		2
...
/* 
 * Read Defect Data 12-byte CDB:
 */
typedef struct dir_read_defect_cdb12 {
	u_char	opcode;			/* Operation Code.		[0] */
	u_char	defect_list_fmt	: 3,	/* Defect List Format.		[1] */
		glist		: 1,	/* Grown Defect List.		    */
		plist		: 1,	/* primary defect list		    */
				: 3;	/* Reserved.			    */
	u_char	reserved_2;		/* Reserved.			[2] */
	u_char	reserved_3;		/* Reserved.			[3] */
	u_char	reserved_4;		/* Reserved.			[4] */
	u_char	reserved_5;		/* Reserved.			[5] */
	u_char	alloc_len3;		/* Allocation Length (MSB).	[6] */
	u_char	alloc_len2;		/*	"	"		[7] */
	u_char	alloc_len1;		/*	"	"		[8] */
	u_char	alloc_len0;		/* Allocation Length (LSB).	[9] */
	u_char	reserved_10;		/* Reserved.		       [10] */
	u_char	link	: 1,		/* Link.		       [11] */
		flag	: 1,		/* Flag.			    */
		naca	: 1,		/* Normal ACA.			    */
			: 3,		/* Reserved.			    */
		vendor	: 2;		/* Vendor Unique.		    */
} DIR_READ_DEFECT_CDB12;

/*
 * Read Defect Data 12-byte Opcode:
 */
#define DIR_READ_DEFECT12_OP	0xB7

/* 
 * Read Defect Data Returned:
 */
typedef struct dir_defect_data12 {
	u_char	reserved_0;		/* reserved			*/
	u_char	defect_list_fmt	:3,	/* Defect list format		*/
		glist		:1,	/* Grown defect list		*/
		plist		:1,	/* primary defect list		*/
				:3;	/* 3 bits reserved		*/
	u_char	list_len3;		/* MSB of list length		*/
	u_char	list_len2;
	u_char	list_len1;
	u_char	list_len0;		/* LSB of list length		*/
		/* 
		 * YES I know I hard coded the numbers..... The index
		 * and sector structs are 8 bytes and the block is
		 * 4 bytes.. With 2 bytes for length (512) max bytes
...
typedef struct dir_defect_list {
	...
	DIR_DEFECT_DESC	defect_desc[128]; 	/* MAXIMUM number	*/
}DIR_DEFECT_LIST;
...
/*
 * Rebuild and Regerate commands
 */

/*
 * Rebuild command cdb 16 byte
TBD - page 65
 */
...
/*
 * Regenerate command cdb 16 byte
TBD - page 67
 */

/*
 * Rebuild and Regerate parameter data
TBD - page 66
 */
...
/*
 * Release command  - These commands are here only for backward
 *	compatability.  The reserve/release in scsi_all.h should be used
 *	for SCSI-3 (or later).
 */
...
/* 
 * Reserve command  - These commands are here only for backward
 *	compatability.  The reserve/release in scsi_all.h should be used
 *	for SCSI-3 (or later).
 */
...
typedef struct dir_extend_data {
	DIR_EXTEND_FORMAT	extend_data[64];	/* Maximum number*/
}DIR_EXTEND_DATA;
...
/*
 * REZERO Unit Command: - Obsolete
 */
typedef struct dir_rezero_cdb6 {
	u_char	opcode;		/* Operation code.		    [0] */
	u_char		: 5,	/* Reserved.			    [1] */
		lun	: 3;	/* Logical Unit Number.			*/
	u_char		: 8;	/* Reserved.			    [2]	*/
...
#define DIR_REZERO_OP	0X01	/* Rezero operation code.		*/
...
typedef struct dir_set_limits_cdb10 {
	u_char	opcode;			/* Operation code.		[0] */
	u_char	WrInh	: 1,		/* Write inhibit.		[1] */
		RdInh	: 1,		/* Read inhibit.	            */
			: 3,		/* Reserved. 		            */
		lun	: 3;		/* Logical unit number.	            */
	...
	u_char	lbaddr0;		/*    "      "      "		[5] */
	u_char		: 8;		/* Reserved.			[6] */
	u_char	num_blocks1;		/* Number of blocks (MSB).	[7] */
	u_char	num_blocks0;		/* Number of blocks (LSB).	[8] */
	u_char	link	: 1,		/* Link.			[9] */
		flag	: 1,		/* Flag.			    */
		naca	: 1,		/* Normal ACA.			    */
			: 3,		/* Reserved.			    */
		vendor	: 2;		/* Vendor Unique.		    */
} DIR_SET_LIMITS_CDB10;

#define DIR_SET_LIMITS_OP	0x33	/* Set Limits opcode. */
...
#define DIR_EJECT_MEDIA		1	/* Eject media from unit.	*/

/*
 * Power condition modes
 */
#define DIR_POWER_NO_CHANGE		0x0
#define DIR_GOTO_ACTIVE			0x1
#define DIR_GOTO_IDLE			0x2
#define DIR_GOTO_STANDBY		0x3
#define DIR_GOTO_SLEEP			0x5
#define DIR_LET_DEVICE_MANAGE		0x7
#define DIR_IDLE_TIMER_TO_ZERO		0xa
#define DIR_STANDBY_TIMER_TO_ZERO	0xb
 ...
 */
typedef struct dir_synch_cache_cdb10 {
        u_char  opcode;                 /* Operation Code.              [0] */
        ...
        u_char  lbaddr0;                /*    "      "      "           [5] */
        u_char          : 8;            /* Reserved.                    [6] */
        u_char  numblks1;               /* MSB number of blocks.        [7] */
        u_char  numblks0;               /* LSB number of blocks.        [8] */
        u_char  control;                /* The control byte             [9] */
}DIR_SYNC_CACHE_CDB10;

#define DIR_SYNC_CACHE_OP	0x35
...
/*
 * Take an lbn and place it into a 10 byte cdb DIRLBN_TO_WRITE_VRFY10
 */
...
#define DIRLBN_TO_WRITE_VRFY10(lbn,cdb); { \
	...
	tmp.l = (lbn); \
	((DIR_WRITE_VRFY_CDB10 *)(cdb))->lbn0 = tmp.c[0]; \
	((DIR_WRITE_VRFY_CDB10 *)(cdb))->lbn1 = (tmp).c[1]; \
	((DIR_WRITE_VRFY_CDB10 *)(cdb))->lbn2 = (tmp).c[2]; \
	((DIR_WRITE_VRFY_CDB10 *)(cdb))->lbn3 = (tmp).c[3]; \
}
/*
 * Place transfer count into a 10 byte cdb DIRTRANS_TO_WRITE_VRFY10
 */
...
#define DIRTRANS_TO_WRITE_VRFY10(count,cdb); { \
	...
	tmp.l = (count); \
	((DIR_WRITE_VRFY_CDB10 *)(cdb))->tran_len0 = tmp.c[0]; \
	((DIR_WRITE_VRFY_CDB10 *)(cdb))->tran_len1 = tmp.c[1]; \
}
 ...
 * WRITE SAME Command
 */
typedef struct dir_write_same_cdb10 {
	u_char	opcode;		/* Operation code.		    [0]	*/
	u_char	reladr	: 1,	/* Relative address.		    [1] */
		lbdata	: 1,	/* Logical block data.			*/
		pbdata	: 1,	/* Physical block data.			*/
			: 2,	/* Reserved.				*/
		lun	: 3;	/* Logical unit number.			*/
	u_char	lbn3;		/* MSB of logical block number.	    [2]	*/
	u_char	lbn2;		/* MID HIGH of logical block number [3] */
	u_char	lbn1;		/* MID LOW of logical block number. [4]	*/
	u_char	lbn0;		/* LSB of logical block number.	    [5] */
	u_char		: 8;	/* Reserved.			    [6]	*/
	u_char	tran_len1;	/* MSB Transfer length in blocks.   [7]	*/
	u_char	tran_len0;	/* LSB Transfer length in blocks.   [8]	*/
	u_char	control;	/* Control byte.		    [9]	*/
} DIR_WRITE_SAME_CDB10;

/* 
 * Write Same 10 byte opcode
 */
#define DIR_WRITE_SAME_OP	0x41

/* ---------------------------------------------------------------------- */

/*
 * XDREAD command cdb 10 byte
TBD - page 77
 */

/*
 * XDWRITE command cdb 10 byte
TBD - page 78
 */

/*
 * XDWRITE EXTENDED command cdb 16 byte
TBD - page 79
 */

/*
 * XPWRITE command cdb 10 byte
TBD - page 80
 */
 ...
 */
typedef struct dir_translate_send_pg {
	u_char		pg_code;	/* Page code 0x40		    */
	u_char			:8;	/* Reserved			    */
	u_char		pg_len1;	/* MSB of page length		    */
	u_char		pg_len0;	/* LSB of page length		    */
	u_char		in_fmt	:3,	/* Supplied (in) format		    */
				:5;	/* Reserved			    */
	u_char		out_fmt	:3,	/* Translated (out) format	    */
				:5;	/* Reserved			    */
	union {
		DIR_DEFECT_BLOCK	address_block;
		DIR_DEFECT_INDEX	address_index;
		DIR_DEFECT_SECTOR	address_sector;
	}address_desc;
}DIR_TRANSLATE_SEND_PG;

/* 
 * The format codes are the DIR_BLOCK_FMT, DIR_INDEX_FMT, and DIR_SECTOR_FMT
 ...
 */
typedef struct dir_translate_rec_pg {
	u_char		pg_code;	/* Page code 0x40		    */
	u_char			:8;	/* Reserved			    */
	u_char		pg_len1;	/* MSB of page length		    */
	u_char		pg_len0;	/* LSB of page length		    */
	...
	u_char		out_fmt	:3,	/* Translated (out) format	    */
				:2,	/* Reserved			    */
			alttrk	:1,	/* Alternate track		    */
			altsec	:1,	/* Alternate sector		    */
			rarea	:1;	/* Reserved area		    */
	union {
		DIR_DEFECT_BLOCK	address_block;
		DIR_DEFECT_INDEX	address_index;
		DIR_DEFECT_SECTOR	address_sector;
	}address_desc;
}DIR_TRANSLATE_REC_PG;

/* 
 * Device Status page - Send Diagnostic
 */

/* 
 * Device Status page - Receive Diagnostic
 */
...
#define DIR_PG_XOR		0x10	/* XOR control mode page	*/
#define DIR_PG_CTRL_OPS		0x22	/* Vendor unique (HSG/V)	*/
					/*  for handling NO SENSE retries */
 ...
 * Medium type codes
 */
#define DIR_DEFAULT_TYPE		0x0
#define DIR_FLEXI_UNKN_1SIDE		0x1
#define DIR_FLEXI_UNKN_2SIDE		0x2
#define DIR_FLEXI_8IN_LD_1SIDE		0x5
#define DIR_FLEXI_8IN_LD_2SIDE		0x6
#define DIR_FLEXI_8IN_HD_1SIDE		0x9
#define DIR_FLEXI_8IN_HD_2SIDE		0xa
#define DIR_FLEXI_5IN_LD_48T_1SIDE	0xd
#define DIR_FLEXI_5IN_MD_48T_2SIDE	0x12
#define DIR_FLEXI_5IN_MD_96T_2SIDE	0x16
#define DIR_FLEXI_5IN_HD_96T_2SIDE	0x1a
#define DIR_FLEXI_3IN_MD_135T_2SIDE	0x1e

/*
 * Direct access tape types
 */
#define DIR_TAPE_12T			0x40
#define DIR_TAPE_24T			0x44
...
typedef struct dir_cache_pg {
	...
	u_char		rcd	:1,	/* read cache disable		*/
			...
			size	:1,	/* Use segement size		*/
			disc	:1,	/* Discontinuities allowed	*/
			cap	:1,	/* Cache analysis permitted	*/
	...
	u_char			:3,	/* Reserved			*/
			...
			lbcss	:1,	/* LBA cache segment size	*/
			fsw	:1;	/* Force sequential write	*/
	u_char		num_seg;	/* number of cache segements	*/
	u_char		c_seg_size1;	/* MSB of cache segment size	*/
	u_char		c_seg_size0;	/* LSB of cache segment size	*/
	u_char			:8;	/* Reserved			*/
	u_char		nc_seg_size2;	/* MSB of non-cache segment size*/
	u_char		nc_seg_size1;	/* Middle of non-cache segment size*/
	u_char		nc_seg_size0;	/* LSB of non-cache segment size*/
}DIR_CACHE_PG;

#define DIR_CACHE_NO_PRIORITY	0x0	/* no data has priority		*/
#define	DIR_CACHE_PREFETCH_HIGH	0x1	/* Pre-fetch data has higher	*/
					/*   priority than I/O data	*/
#define DIR_CACHE_PREFETCH_LOW	0xf	/* I/O data has higher priority */
					/*   than pre-fetch data	*/
...
typedef struct dir_flexi_pg{
	...
	u_char	pin2    :4,	/* pin 2 of interface			*/
		pin34   :4;	/* pin 34 of interface			*/
	u_char	pin1    :4,	/* pin 1 of interface			*/
		pin4    :4;	/* pin 4 of interface			*/
...
/*
 * A few examples of transfer rate values.
 */
#define DIR_FLEXI_XFER_250KB	0xfa	/* 250Kb/second			*/
#define DIR_FLEXI_XFER_300KB	0x12c	/* 300Kb/second			*/
#define DIR_FLEXI_XFER_500KB	0x1f4	/* 500Kb/second			*/
#define DIR_FLEXI_XFER_1Mb	0x3ea	/* 1Mb/second			*/
#define DIR_FLEXI_XFER_2Mb	0x7d0	/* 2Mb/second			*/
#define DIR_FLEXI_XFER_5Mb	0x1388	/* 5Mb/second			*/
...
/* 
 * Defines for the rotational position locking field. (octal)
 *	(Another name for spindle sync)
 */
 ...
 */
typedef struct dir_xor_pg {
	DIR_MODE_PG_HD	pg_head;	/* Page header			*/
	u_char			:1,	/* Reserved			*/
		xordis		:1,	/*				*/
				:6;	/* Reserved			*/
	u_char			:8;	/* Reserved			*/
	u_char	max_xor_size3;		/* MSB - maximum xor write size */
	u_char	max_xor_size2;		/*   Middle max xor write size	*/
	u_char	max_xor_size1;		/*   Middle max xor write size	*/
	u_char	max_xor_size0;		/* LSB - maximum xor write size */
	u_char			:8;	/* Reserved			*/
	u_char			:8;	/* Reserved			*/
	u_char			:8;	/* Reserved			*/
	u_char			:8;	/* Reserved			*/
	u_char	max_regen_size3;	/* MSB - maximum regerate size	*/
	u_char	max_regen_size2;	/*   Middle max regerate size	*/
	u_char	max_regen_size1;	/*   Middle max regerate size	*/
	u_char	max_regen_size0;	/* LSB - maximum regerate size	*/
	u_char	max_rebld_size3;	/* MSB - max rebuild read size	*/
	u_char	max_rebld_size2;	/*   Middle rebuild read size	*/
	u_char	max_rebld_size1;	/*   Middle rebuild read size	*/
	u_char	max_rebld_size0;	/* LSB - max rebuild read size	*/
	u_char			:8;	/* Reserved			*/
	u_char			:8;	/* Reserved			*/
	u_char	rebuild_delay1;		/* MSB of rebuild delay		*/
	u_char	rebuild_delay0;		/* LSB of rebuild delay		*/
}DIR_XOR_PG;
 ...
 *	This is a vendor unique mode page for HSG/V controllers to
 *	enable them to send NO SENSE instead of UA for informational
 *	data to the host.
 */
typedef struct dir_ctrl_ops_pg {
	DIR_MODE_PG_HD	pg_head;	/* Page header			*/
	u_char	rnsc	:1,		/* Recoverable No-sense capable */
		rnse	:1,		/* Recoverable No-sense enabled */
			:6;		/* Reserved			*/
	u_char		:8;		/* Reserved			*/
	u_char		:8;		/* Reserved			*/
	u_char		:8;		/* Reserved			*/
