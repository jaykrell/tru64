 ...
 */
/*
 * @(#)$RCSfile: aceregs.h,v $ $Revision: 1.1.49.1 $ (DEC) $Date: 2002/01/24 18:54:31 $
 */
...
struct ace_saved_reg {
  ...
  int dllsb;	/* Baud Rate lsb */
  int dlmsb;	/* Baud Rate msb */
  int ie;	/* Interrupt Enable Register */
};

/* Driver and data specific structure */
struct ace_softc {
    u_long		  ace_regs;		/* Base register address */
    ihandler_id_t	  *ace_id_t;		/* Handler id */
    int			  ace_type;		/* Device type flag */
    int			  ace_fifo_size;	/* Size of xmit fifo */
    long		  ace_flags;		/* Flags */
    long		  ace_category_flags;	/* Category flags */
    u_long		  ace_softcnt;		/* Soft error count */
    u_long		  ace_hardcnt;		/* Hard error count */
    u_char		  ace_modem;		/* Software modem state */
    u_char		  ace_brk;		/* State of break condition */
    struct timeval	  ace_timestamp;	/* Used for DCD drop timer */
    struct ace_saved_reg  ace_saved_regs;	/* Save register state */
    struct card_info      *cinfop;		/* Ptr to card if pcmcia */
    MSS                   *ace_mss;		/* Device's Management */
						/* Support Structure. */
    void		  *ace_current_values;	/* Pointers to device's */
						/* current EHM attributes */
    void		  *ace_dev_handle;	/* Device handle returned */
						/* by hwc_reg_hw() call */
    struct hwc_tty_ehm_attributes ace_ehm_attr; /* EHM attribute table */
};

/* Flag definitions for ace_flags. */
#define ACE_FLAGS_PIC_LEVEL_SENSITIVE 0x0001    /* =1 level sensitive */
						/* =0 edge triggered */
...
#define ACE_B200_LO	 0x40
#define ACE_B200_HI	 0x02
...
#define ACE_B57600_LO    2
#define ACE_B57600_HI    0

#define ACE_B115200_LO   1
#define ACE_B115200_HI   0
