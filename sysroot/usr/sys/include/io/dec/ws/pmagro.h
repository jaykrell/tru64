 ...
 */
/*	
 *	@(#)$RCSfile: pmagro.h,v $ $Revision: 1.2.9.2 $ (DEC) $Date: 1999/04/05 11:41:03 $
 */ 
/*
 */
/*
 ...
 * function protos
 */
extern int pmagro_attach(register struct controller *ctlr);
extern void pmagro_interrupt(register struct controller *ctlr, 
			     caddr_t closure);
extern void pmagro_bt463_enable_interrupt(caddr_t closure);
extern void pmagro_bt431_enable_interrupt(caddr_t closure);
extern caddr_t pmagro_bt431_init_closure(caddr_t closure, caddr_t address, 
					 int unit, int type);
extern caddr_t pmagro_bt463_init_closure(caddr_t closure, caddr_t address, 
					 int unit, int type);
extern int pmagro_recolor_cursor(caddr_t closure, 
				 ws_screen_descriptor *screen, 
				 ws_color_cell *fg, ws_color_cell *bg);
extern int pmagro_video_off(caddr_t closure);
extern int pmagro_video_on(caddr_t closure);
extern int pmagro_map_screen(caddr_t closure, ws_depth_descriptor *depths, 
			     ws_screen_descriptor *screen, ws_map_control *mp);
extern int pmagro_load_cursor(caddr_t closure, ws_screen_descriptor *screen, 
			      ws_cursor_data *cursor, int sync);
extern int pmagro_ioctl(caddr_t closure, u_int cmd, caddr_t data, int flag);
