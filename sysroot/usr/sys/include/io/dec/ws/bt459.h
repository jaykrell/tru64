 ...
 */
/*	
 *	@(#)$RCSfile: bt459.h,v $ $Revision: 1.2.19.2 $ (DEC) $Date: 1999/04/05 11:40:58 $
 */ 
/*
 */
/*
...
#define bt_addr_low(V,A)        { (V)->addr_low=_DUPBYTE0(A); mb(); }
#define bt_addr_high(V,A)       { (V)->addr_high=_DUPBYTE1(A); mb(); }
#define bt_addr_set(V,A)        { (V)->addr_high=_DUPBYTE1(A); mb(); \
                                  bt_addr_low(V,A); }

#define bt_dup0_reg(V,I)        { (V)->bt_reg = _DUPBYTE0(I); mb(); }
#define bt_dup1_reg(V,I)        { (V)->bt_reg = _DUPBYTE1(I); mb(); }

#define bt_dup0_map(V,I)        { (V)->color_map=_DUPBYTE0(I);mb(); }
#define bt_dup1_map(V,I)        { (V)->color_map=_DUPBYTE1(I);mb(); }
#define bt_dup2_map(V,I)        { (V)->color_map=_DUPBYTE2(I);mb(); }
 ...
 * function protos
 */
extern caddr_t bt_init_closure(caddr_t closure, caddr_t address, int unit, 
			       int type);

extern int bt_load_formatted_cursor(register struct bt459info *bti);
extern int bt_recolor_cursor(caddr_t closure, ws_screen_descriptor *screen, 
			     ws_color_cell *fg, ws_color_cell *bg);
extern int bt_set_cursor_position(caddr_t closure, ws_screen_descriptor *sp, 
				  register int x, register int y);
extern int bt_load_cursor(caddr_t closure, ws_screen_descriptor *screen, 
			  ws_cursor_data *cursor, int sync);
extern int bt_load_color_map_entry(caddr_t closure, int map, 
				   register ws_color_cell *entry);
extern int bt_get_unit(caddr_t closure);

extern int bt_init_color_map(caddr_t closure);
extern int bt_cursor_on_off(caddr_t closure, int on_off);
extern int bt_video_off(caddr_t closure);
extern int bt_video_on(caddr_t closure);
extern void bt_clean_colormap(caddr_t closure);

extern void bt_wb_nonmerging(void);
extern void bt_wb_merging(void);


extern caddr_t bt459_2_init_closure(caddr_t closure, caddr_t address, 
				    int unit, int type);
extern int bt459_2_load_formatted_cursor(register struct bt459info2 *bti);
extern int bt459_2_recolor_cursor(caddr_t closure, 
				  ws_screen_descriptor *screen, 
				  ws_color_cell *fg, ws_color_cell *bg);
extern int bt459_2_load_cursor(caddr_t closure, 
			       ws_screen_descriptor *screen, 
			       ws_cursor_data *cursor, int sync);
extern int bt459_2_load_color_map_entry(caddr_t closure, int map, 
					register ws_color_cell *entry);
extern int bt459_2_init_color_map(caddr_t closure);
extern void bt459_2_clean_colormap(caddr_t closure);
extern int bt459_2_cursor_on_off(caddr_t closure, int on_off);
