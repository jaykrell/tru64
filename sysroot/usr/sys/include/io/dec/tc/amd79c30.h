 ...
 */
/*
 * @(#)$RCSfile: amd79c30.h,v $ $Revision: 1.1.8.2 $ (DEC) $Date: 1997/06/24 13:48:16 $
...
/*
 * The interrupt service array for loading/unloading the dlld module
 * (isdn_bba.mod0.
 */
typedef struct amd_ISR_array_s {
	void (*isdn_dscisr)();
	int (*dlld_ioctl)();
	int (*dlld_mproto)();
	int (*dlld_unix_close)();
...
/*
 * The extern structure which can be accessed from outside, isdn_bba.mod
 * reconfigures these pointers to point to its own code when it the configure
 * routine is called
 */
extern amd_ISR_array_t amd_ISR_array;

/*
 * Reset function for when isdn_bba.mod gets unconfigured.
 */
extern int reset_amd_ISR_array(void);
...
#ifndef NUM_TX_BUFS
#define NUM_TX_BUFS	2	/* Output HDLC transmssion buffers	*/
#endif
...
struct BBA_open {
	...
	ushort bo_congestion;		/* Congestion Mark */
	ushort bo_curmark;		/* No of bytes current on STREAMS Q */
	...
	struct hdlc_info bo_hdlcout[NUM_TX_BUFS];/* Output HDLC		*/
};
...
#define	BBACONGESTION		1	/* M_CTL arg returned for congestion */
