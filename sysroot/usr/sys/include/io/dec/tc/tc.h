 ...
 */
/*	
 *	@(#)$RCSfile: tc.h,v $ $Revision: 1.2.25.1 $ (DEC) $Date: 2002/01/24 18:54:40 $
 */ 
/*
...
#define tc_intr_info	handler_intr_info
...
struct tc_sw {
	...
	int (*option_control)();  /* routine to set tc operating modes */
};


extern void tc_init(void);
extern caddr_t tc_slot_to_addr(int slot);
extern char * tc_slot_to_name(int slot);

extern struct tc_sw tc_sw;

struct controller; /* global scope definition suppresses protoscope warnings */
...
void tc_enable_option(struct controller *);
...
void tc_disable_option(struct controller *);
...
int	tc_option_control(struct controller *, int);
...
int	tc_module_name(struct controller *, char *);
...
int	tc_addr_to_name(vm_offset_t, char *);
...
int	tc_isolate_memerr(struct tc_memerr_status *);
