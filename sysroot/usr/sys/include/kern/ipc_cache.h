 ...
 */
/*	
 *	@(#)$RCSfile: ipc_cache.h,v $ $Revision: 4.2.6.2 $ (DEC) $Date: 1999/02/09 19:03:06 $
 */ 
/*
 */
/*
...
MACRO_BEGIN								\
	unsigned int __obj_index = obj_cache_index(_name);		\
									\
	(task)->obj_cache[__obj_index].name = (_name);			\
	(task)->obj_cache[__obj_index].object = (_object);		\
MACRO_END
...
MACRO_BEGIN								\
	unsigned int __obj_index = obj_cache_index(_name);		\
									\
	if ((task)->obj_cache[__obj_index].name == (_name))		\
		obj_cache_clear((task), __obj_index);			\
MACRO_END
...
MACRO_BEGIN								\
	unsigned int __obj_index = obj_cache_index(_name);		\
									\
	if ((task)->obj_cache[__obj_index].name == (_name)) {		\
		(_object) = (task)->obj_cache[__obj_index].object;	\
