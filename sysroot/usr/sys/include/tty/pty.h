 ...
 */
/*
 * @(#)$RCSfile: pty.h,v $ $Revision: 1.1.31.5 $ (DEC) $Date: 1997/07/23 13:43:11 $
 */
/*
...
struct pty_s {
	...
	int			pt_bufcall_id;  /* pty bufcall id       */
...
#define PF_HANGUP	0x00004000	/* master has sent hangup msg */
#define PF_STOPDRAIN    0x00008000      /* STOP "close draining" */
...
#define SLAVE_HIWATER		2048
#define SLAVE_LOWATER		1024

#define BSD_MAST_HIWATER	2048
#define BSD_MAST_LOWATER	1024

#define SYSV_MAST_HIWATER	2048
#define SYSV_MAST_LOWATER	1024
...
#define PTY_OPCL_LOCK_INIT()	lock_setup(&PTY_OPCL_lock,pty_opcl_lockinfo,1)
...
#define PTY_WRITE_LOCK(ptp)						    \
	while (1) {							    \
		...
		if ((ptp)->pt_readers != 0) {				    \
			...
			assert_wait_mesg((vm_offset_t) &(ptp)->pt_readers,  \
							FALSE,ptyclck);     \
...
#define PTYPRI PZERO			/* sleep priority		*/
...
int
ptm_configure(cfg_op_t, caddr_t, ulong, caddr_t, ulong);
...
#ifdef __digital__
int
ptrselect(dev_t, int *, int *, int, void *);
#else
int
...
int
pts_configure(cfg_op_t, caddr_t, ulong, caddr_t, ulong);
