 ...
 */
/*	
 *	@(#)$RCSfile: ioctl_compat.h,v $ $Revision: 4.5.47.1 $ (DEC) $Date: 2002/01/24 18:41:06 $	
 */ 
/*
 */
/*
...
#define DEVROOT		_IOR('v', 4, struct console_device)	/* Set rootdev */
#define DEVCONSOLE	_IOR('v', 7, dev_console_t)	/* Get console dev info */

struct ioc_drvhandle {
        void    *drv_handle;
};

#define DRV_GETHANDLE   _IOR('v', 8, struct ioc_drvhandle) /* Get a drv's handle
                                                              so no lookup
                                                              needed after open */

#define DEVPATHAVAIL 	_IO('v', 9 )		/* Get path availability */
					/* Get console dev info for all paths */
#define DEVCONSOLE_ALLPATHS _IOWR('v', 10, dev_console_allp_t)

struct ioc_dev_control {
	u_long	ctrl_opcode;			/* Operation code see below */

#define CTRL_RESET_DEV	0x0000000000000001	/* Reset device (eg, lun 
						   reset SCSI) Note not
						   all devices support */ 

	u_int	reserved[8];			/* Must be zeroed when
						   Ioctl is issued - Future
						   expansion */
};

/* 
 * Issue a device control command (DEV_CONTROL) to a specific device.  
 * Warning  this IOCTL should only be issued by kernel components 
 * (e,g,. cluster  control modules (drd) and certain file system 
 * components (ADVfs)
 */ 
#define DEV_CONTROL	_IOWR('v', 11, struct ioc_dev_control )

#define	CAM_DT_CFCS	_IO('!', 1)	/* Clear failed CAM State */

/* Generic SCSI tape command passthrough */
#define SCSI_TP_GENERIC_PASSTHRU  _IOWR('v', 12, struct tp_generic_pass)
 ...
 */
struct tp_generic_pass {
  unsigned int  flags;		/* See CTAPE_GEN_PASS_XX defs below @   */
  void          *cdb;           /* SCSI command data block/TMF      @   */
...
#define	CTAPE_GEN_PASS_DIRMASK	0x01
#define	CTAPE_GEN_PASS_READ	0
#define	CTAPE_GEN_PASS_WRITE	1

/* Setting this bit in the flags field of the tp_generic_pass struct
 * changes the meaning of the cdb field to one of the
 * Task Management Functions below.
 */
#define	CTAPE_GEN_PASS_TMF_MODE	0x04

#define CTAPE_GEN_TARGET_RESET_CURRENT_PATH	0x10
#define CTAPE_GEN_TARGET_RESET_ALL_PATHS	0x11
#define CTAPE_GEN_LUN_RESET_CURRENT_PATH	0x20
#define CTAPE_GEN_LUN_RESET_ALL_PATHS		0x21

/* SCSI tape path locking/unlocking ioctl command
 * Prevents or allows the tape driver selecting a new path during open.
 */
#define SCSI_TP_PATH_CONTROL  _IOW('v', 13, int)

#define CTAPE_UNLOCK_CURRENT_PATH	0
#define CTAPE_LOCK_CURRENT_PATH		1

/*
 * ioctl to obtain device information
 */
#define DEVGETINFO	_IOR('v', 1, device_info_t)

/* re/SWXCR ioctl's for RAID configuration */
#define DIODCMD		_IOWR('v', 5, XCR_DIRECTCMD)	/* Direct cmd */
#define DIODCDB		_IOWR('v', 6, XCR_SCSICMD)	/* Direct scsi cmd */
