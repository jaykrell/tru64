 ...
 */
/*
 * This software is provided "as is" into the public domain 
 ...
 *
 * As  listed in "A STREAMS-based Data Link Provider Interface - Version 2"
 */
...
/*
** Intergraph additional primitives for X.25 data request & indications
** in CODLS.
*/

#define DL_X25DATA_REQ		0x99	/* X.25 CO data send request */
#define DL_X25DATA_IND		0x9a	/* Arrival of an XID PDU */
...
/*
** Intergraph additions
*/
#define DL_PRIMSZ_UDERR  0x80	/* bad protocol message size in udreq */
#define DL_ADDRSZ_UDERR	 0x81	/* bad address size in unitdata request */
#define DL_DATASZ_UDERR	 0x82	/* bad data size in unitdata request */
#define DL_HDRPAD_UDERR  0x83	/* no room for Ethernet header */
...
#define DL_ASYN         0x0a	/* Bit Asynchronous Serial Interface */
...
typedef struct {
	...
	ulong	dl_subs_sap_len;	/* length of subs_sap */
#define dl_subs_sap_length	dl_subs_sap_len
...
typedef struct {
	...
	ulong	dl_correct_primitive;	/* primitive being acknokwledged */
} dl_ok_ack_t;
...
typedef struct {
	...
	ulong	dl_level;		/* physical,SAP level or ALLmulticast*/
} dl_promiscon_req_t;
...
/*
** Intergraph additional X.25 primitives; CODLS data req/ind
*/

/*
 * DL_X25DATA_REQ, M_PROTO type
 */
typedef struct {
	ulong	dl_primitive;		/* DL_X25DATA_REQ */
	ulong	dl_control;		/* control bits */
} dl_x25data_req_t;

/*
 * DL_X25DATA_IND, M_PROTO type
 */
typedef struct {
	ulong	dl_primitive;		/* DL_X25DATA_IND */
	ulong	dl_control;		/* control bits */
} dl_x25data_ind_t;
...
union DL_primitives {
	...
	dl_x25data_req_t	x25data_req;
	dl_x25data_ind_t	x25data_ind;
};
...
#define DL_X25DATA_REQ_SIZE	sizeof(dl_x25data_req_t)
#define DL_X25DATA_IND_SIZE	sizeof(dl_x25data_ind_t)


/*
** Ethernet type definitions
*/
#define	ISO_SAP		0x0000		/* for ISO/OSI protocol		*/
#define MAX_ISO_SAP	0x05DC		/*     (0x0000 - 0x05DC)	*/
#define	XNS_SAP		0x0600		/* for XNS protocol		*/
#define	DOD_SAP		0x0800		/* for DOD (TCP/IP) protocol	*/
#define ARP_SAP		0x0806		/* for ARP protocol		*/
#define RARP_SAP	0x8035		/* for RARP protocol		*/
#define TRLR_SAP	0x1001		/* for IP-Trailer protocol	*/
#define MAX_TRLR_SAP	0x1010		/*     (0x1001 - 0x1010)	*/

#define MAX_SAP		0xFFFF		/* maximum allowable SAP	*/
#define UNDEF_SAP	(-1)		/* for uninitialized drivers	*/
#define PROMISC_SAP	(-2)		/* for promiscuous SAP		*/
#define LOCAL_P_SAP	(-3)		/* all on this node SAP		*/

#define ILLEGAL_SAP(sap)	((sap < 0 || MAX_SAP < sap) ? 1 : 0)


/* msdlpi additions */

/*
 * DL_DATA_REQ, M_PROTO type
 */
typedef struct {
        ulong   dl_primitive;
} dl_data_req_t;
...
#define	DL_PRIM_EXT	0x50

#define	DL_DATA_REQ	(DL_PRIM_EXT + 0)	/* CODLS data reqest */
#define	DL_DATA_IND	(DL_PRIM_EXT + 1)	/* CODLS data indication */

#define	DL_ERR_EXT	0x50

#define	DL_INVADDR	(DL_ERR_EXT + 0)
#define	DL_AUTO		(DL_ERR_EXT + 1)

/*
 * XID and TEST flags
 */

#define	DL_COMMAND	0x0000
#define	DL_RESPONSE	0x0002
