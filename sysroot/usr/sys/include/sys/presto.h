 ...
 */
/*	
 *	@(#)$RCSfile: presto.h,v $ $Revision: 4.2.15.6 $ (DEC) $Date: 1998/05/11 17:57:32 $
 */ 
/*
 ...
 *
 * We choose prbsize is a configurable parameter with a 8192 default that
 * matches the UFS default blocksize.  1024 is the UFS default fragment size.
*/
extern int prbsize;				/* largest presto bsize */
...
#define PRBUFHSZ	256		/* default size unless set in 
					 * sysconfigtab
					 */	
#define PRRND		(prbsize / DEV_BSIZE)

extern int prbufhsz;
extern struct bufhd *prbufhash;
extern struct buf *prbufs;
extern int prnbufs;
extern struct presto_status prstatus;

/* Fudge the "device" bits to hash on */
#define DEV_TARGET(dev)		(((GETUNIT(dev)) >> 4) & 0x07)

#define PRBUFHASH(dev, dblkno) \
	((struct buf *)&prbufhash \
		[((u_int)(DEV_TARGET(dev))+(((int)(dblkno))/PRRND)) & (prbufhsz-1)])
...
enum battery_modes {
	BAT_MODE_NONE 	 = 0,		/* batteries are always bad */
	BAT_MODE_BAD 	 = 1,		/* batteries are bad (and tell them) */
	BAT_MODE_FAKE 	 = 2,		/* pr_fakebattery used */
	BAT_MODE_REAL 	 = 3		/* Just The Batteries Mam */
};
