 ...
 */
/*	
 *	@(#)$RCSfile: prestoioctl.h,v $ $Revision: 4.2.17.5 $ (DEC) $Date: 1999/02/26 20:05:29 $
 */ 
/*
 */
/*
...
struct prtab {
	major_t pt_major;		/* driver's major number */
	struct dsent *pt_dsent;		/* driver's device switch entry */
	char  *pt_driver_name;		/* driver's name */
	int    pt_rinstance;		/* driver's major number instance */
	struct prunit *pt_unit;		/* enabled unit chain */
	struct prunit *pt_hunit[NPRUHASH];
					/* enabled unit hash chains */
};
...
struct prunit {
	...
	long	      pu_max_clusterio; /* 48: per unit max cluster io size */
};
